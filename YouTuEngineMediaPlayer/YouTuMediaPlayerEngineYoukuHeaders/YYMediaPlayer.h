...
typedef NS_ENUM(NSInteger, YYErrorCode) {
    ...
    YYErrorInvalidClient    = -109,      // 无效的client_id
...
/**
 * @功能 播放指定 url 地址的视频
 * @参数 url:视频地址, 可以为本地或网络视频文件地址
 * @参数 from:播放起始点
 * @备注 支持绝大多数主流视频文件, 包括 avi, mov, mp4, flv, rm, 3pg, m3u8 等;
 * @备注 该接口的相关状态尚未完备, 暂不提供
 */
//- (void)playUrl:(NSURL *)url from:(NSTimeInterval)from;
...
- (void)showFooterAd;                               //显示角标
- (void)hideFooterAd;                               //隐藏角标

- (void)showQuestionnaireView;                      //显示调查问卷
- (void)hideQuestionnaireView;                      //隐藏调查问卷

- (void)pauseQuestionTimer;                         //暂停调查问卷的倒计时
- (void)resumeQuestionTimer;                        //恢复调查问卷的倒计时

+ (void)startOfflineAdTask;                         //开始离线广告任务
...
@interface YYMediaPlayer (YYMediaPlayerAirplay)

@property (nonatomic, getter = isAirplayEnabled) BOOL airplayEnabled;          // 是否打开AirPlay功能,默认为关闭
@property (nonatomic, retain, readonly) NSArray *airplayDeviceList;            // 当前网络中的AirPlay设备列表
@property (nonatomic, retain, readonly) NSString *usingAirplayDevice;          //当前选中的AirPlay设备名称,可能为nil.
@property (nonatomic, readonly, getter = isAirplayPlaying) BOOL airplayPlaying; // 当前播放器是否处于 AirPlay 状态

- (void)startAirplayOnDevice:(NSString *)device;              // 在 device 设备上开始 AirPlay 播放, device 必须从 deviceList 中取得
- (void)stopAirplay;                                          // 停止 AirPlay 播放, 视频回到播放器内继续播放
- (void)verifyAirplayDevicePassword:(NSString *)password;     // 设备需要配对密码时, 发送密码


/**
 * @discussion 直接使用airplay初始化方法
 */
- (id)initWithDirectAirplay;

/**
 * @discussion 根据vid直接在airplay设备上播放视频
 * @params vid          视频vid
 * @params quality      视频清晰度，如果没有默认为标清
 * @params time         视频起播时间
 * @params deviceName   投放的设备名称
 * @params deviceIP     投放的设备IP地址
 * @params port         投放的端口号
 */
- (void)startAirplayWithVid:(NSString *)vid
                    quality:(NSString *)quality
                       from:(NSTimeInterval)from
                 deviceName:(NSString *)deviceName
                   deviceIP:(NSString *)deviceIP
                 devicePort:(NSInteger)port;
...
@interface YYMediaPlayer (YYMediaPlayerLive)

@property (nonatomic, copy) NSString *pageCode;             //设置页面编码，默认"-1"
@property (nonatomic, copy) NSString *referencePageCode;    //设置页面来源编码，默认"-1"
@property (nonatomic,readonly) YYLiveBarrageSwitch  barrage;         //设置弹幕开关
@property (nonatomic,readonly) YYLiveBarrageDensity density;         //设置弹幕密集度

@property (nonatomic,readonly) YYLiveVideoState livestate;          // 0：直播未开始，1：正在直播，2：直播已结束
@property (nonatomic,readonly) YYLiveVideoStateLimit livelimitState;    // 当收到 YYErrorLiveVideoStateLimit 消息的时候需要处理的信息
@property (nonatomic, retain, readonly) NSString *lvlimitMessage;   //视频限制提示语

@property (nonatomic, retain, readonly) NSString *lvid;             //liveid
@property (nonatomic, retain, readonly) NSString *lvsid;            //sessionid

@property (nonatomic, retain, readonly) NSString *lvstarttime;      //预计直播开始时间 时间戳格式：1395741000
@property (nonatomic, retain, readonly) NSString *lvendtime;         //预计直播结束时间 时间戳格式：1395741000

@property (nonatomic, retain, readonly) NSString *lvbegintime;      //预计直播开始时间 时间戳格式：20:00
@property (nonatomic, retain, readonly) NSString *lvfinishtime;     //预计直播结束时间 时间戳格式：20:00

@property (nonatomic, readonly) NSInteger lvcountdowntime;          //直播倒记时时间单位秒，-10000000：无效

@property (nonatomic, retain, readonly) NSString *lvvideotitle;      //直播视频标题
@property (nonatomic, retain, readonly) NSString *lvtitle;          //测试标题
@property (nonatomic, retain, readonly) NSString *lvdescription;    //描述
@property (nonatomic, retain, readonly) NSString *lvservertime;     //服务器时间   时间戳格式：1395741000
@property (nonatomic, retain, readonly) NSString *lvuid;            //uid
@property (nonatomic, retain, readonly) NSString *lvtotaltime;      //总时长
@property (nonatomic, retain, readonly) NSString *lvchannelid;      //频道id
@property (nonatomic, retain, readonly) NSString *lvvip;            //0：非vip，1:vip
@property (nonatomic, retain, readonly) NSString *lvvipnotes;       //当前vip用户如果超过分享限制时的提示文字

@property (nonatomic, readonly) NSInteger lvbarrage;                //是否有弹幕，0无，1有
@property (nonatomic, readonly) NSInteger lvbarrageid;              //弹幕id
@property (nonatomic, readonly) NSInteger lvshowcountdown;          //1:显示，0:不显示
@property (nonatomic, retain, readonly) NSArray *lvbarrageColor;    //弹幕颜色值

- (void)playLiveVideo:(NSString *)liveId;
- (void)stopLiveVideo;                                              //仅在播放过程，或者是暂停的时候，登录成功，刷新播放器的时候调用。

@end


@interface YYMediaPlayer (YYMediaPlayerLiveStatistic)

@property (nonatomic, retain, readonly)id livePlayItem;

@end


@interface YYMediaPlayer (YYMidAdTest)

@property (nonatomic) NSTimeInterval midAdForbiddenTime;   // play之后,YYBuildDebug时生效

@property (nonatomic) NSTimeInterval midAdPointAfterTime;  // play之后,YYBuildDebug时生效

@end


@interface YYMediaPlayer (YYUnicom)

- (void)passiveStop;   // 被动停止，可恢复播放，恢复播放可调用replay接口

@end


@interface YYMediaPlayer (YYMediaPlayerStatistic)

@property (nonatomic, retain, readonly) NSDictionary *sidData;    //可能为空.
